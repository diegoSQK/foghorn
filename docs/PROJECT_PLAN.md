...
*Nothing in flight. The July 2026 venue-expansion batch (a user-directed feature-branch push) delivered Phase 5's intent and beyond — 23 new venues across SF, East Bay, and the Peninsula — plus Phase 7.1 (genre). Venue batch 2 shipped July 2026 (UC Theatre, Bimbo's, Neck of the Woods, Warfield, August Hall, Thee Stork Club — 35 venues total); venue batch 3 (Club Deluxe, Club Fox) shipped July 2026 — 37 venues; The Mellow (Haight) + Blue Heron Lake Boathouse shipped August 2026 — **78 seeded venues** — see [SHIPPED.md](SHIPPED.md#the-mellow--eventon-recon--two-room-scraper-august-2026) for the EventON playbook (the `evo-ajax=eventon_get_events` endpoint, its nonce + harvested-shortcode requirements, and the `focus_*_date_range` window that `fixed_month` only pretends to set); Freight & Salvage shipped August 2026 — **79 seeded venues** — reached via its Tessitura TNEW ticketing host after the main site proved Cloudflare-walled, see [SHIPPED.md](SHIPPED.md#freight--salvage--behind-the-cloudflare-wall-via-tessitura-august-2026) (**the `secure.`/`tickets.` CNAME check is the cheapest unexplored move on any venue parked as "blocked"**); **SFJAZZ shipped August 2026 — the last dormant Phase 2 venue, 182 shows** via the Umbraco `ace-api/events` endpoint that appeared in their site rebuild, see [SHIPPED.md](SHIPPED.md#sfjazz--the-last-dormant-phase-2-venue-finally-scraped-august-2026) (note: that scraper deliberately uses stdlib `urllib`, not `httpx`, because Cloudflare fingerprints `httpx` as a bot — a documented fragility, not an oversight); the Ticketmaster venue batch shipped August 2026 — Brick & Mortar, The Masonic, Mountain Winery, The Midway — **83 seeded venues**, see [SHIPPED.md](SHIPPED.md#ticketmaster-venue-batch--4-rooms-132-shows-august-2026) (the coverage audit behind it also lists the remaining gaps by platform, and records Starline Social Club / Thee Parkside / Cafe Stritch as **closed** — don't re-add them); the **August 2026 coverage audit** shipped 8 venues + 1 promotion and closed the sweep — see [SHIPPED.md](SHIPPED.md#coverage-audit--the-august-2026-venue-sweep-and-what-it-closed-august-2026) for the method (reachability proves nothing about whether a venue is open; ticketing inventory does), the closures not to re-add (Starline, Thee Parkside, Cafe Stritch, the Blue Note Napa *club*), and the two documented dead ends (El Rio has no first-party calendar at all; Montalvo is reachable Tessitura but ~5 music events in 38); The New Parish remains blocked on empty TicketWeb inventory (re-verified 2026-07-07 across its site, both TicketWeb venue ids, and aggregators — endpoint pre-solved; re-verified still empty 2026-07-18, recheck ~August) plus a Ticketmaster Discovery API spike for the Live Nation rooms (Fillmore, Regency); see the venue-expansion SHIPPED entry for the per-platform scraping playbook.*
...
- **Venue batch 5 remainder (July 2026).** Tranches A+B shipped 16 of the sweep's greens ([tranche B SHIPPED entry](SHIPPED.md#venue-batch-5-tranche-b-ten-more-from-the-sweep-july-2026) has the per-venue closures). Still open, with dated verdicts: **Fox Theatre RWC** (ShoWare JS — needs its API), **Rio Theatre** + **Sebastiani** (no exposed Squarespace events collection — deeper recon; both low-priority regions), **Rancho Nicasio** (no first-party times — OvationTix API is the unblock), **The Big Easy Petaluma** (IP-keyed 429s as of 2026-07-19; the venue is a jazz/blues jackpot if ever solved), **The Saloon** (hand-lettered PDF defeats OCR; mirror dead), **Sausalito Seahorse** (CalendarWiz "Server busy" gate; re-probe later), plus the earlier yellows (Ashkenaz/Jupiter JS-Squarespace, Ceremony/Mountain Winery ticketing APIs, Angelica's thin first-party, Bazaar Cafe GCal iframe, Monkey House Wix 429s, Royal Cuckoo no-website). Santa Cruz is the user's lowest-priority region — weigh SC items accordingly. **Unblock conditions:** per-item above.
...
- **Hosting / deployment.** ✅ Shipped August 2026 (VPS + Docker Compose + Caddy, deploy-on-merge) — see [Multi-user accounts + VPS deployment](SHIPPED.md#multi-user-accounts--vps-deployment-august-2026) and the [DEPLOY.md](DEPLOY.md) runbook. *Remaining Diego actions:* provision the box, point DNS, set the `DEPLOY_*` repo secrets.
- **Multi-user accounts.** ✅ Shipped August 2026 (invite-link auth, per-user watchlists, admin user management) — see [SHIPPED entry](SHIPPED.md#multi-user-accounts--vps-deployment-august-2026). The fleet deployment runs **single-user** via `FOGHORN_SINGLE_USER=1` (cookie-less requests resolve as the bootstrap admin — the iOS PWA has no way to open an invite link); the VPS keeps full auth and pins the flag off. ✅ Shipped August 2026 — see [SHIPPED entry](SHIPPED.md#single-user-mode-for-the-fleet-deployment-august-2026). Magic-link email recovery stays deferred until "ask Diego for a fresh link" is real friction; an in-app `/signin` route is the separate fix for inviting other people onto phones.
...
- **CORS tighten.** Currently `allow_origins=["*"]`; `FOGHORN_CORS_ORIGINS` tightens. Mostly de-fanged by the August 2026 auth work — browser calls are same-origin via the Next rewrite (and Caddy routes `/api` on the same domain in production), and cookie auth doesn't rely on CORS. Tightening is now hygiene, not a blocker; do it when the VPS goes live.
