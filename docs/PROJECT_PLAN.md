...
- **Little Hill Lounge scraper** (El Cerrito). *Now covered indirectly via Bay Improviser aggregator ingest (July 2026).* Site (littlehillelcerrito.com) posts the calendar as a monthly flyer JPEG; a former Tribe Events install was removed (re-verified 2026-07-17 via the wp-json namespace list — no new widget either); Bandsintown lists it but bot-blocks plain HTTP. **Unblock condition:** venue restores a structured calendar, or OCR/headless-browser ingest is deliberately taken on.
- **Long-tail audit leftovers (July 2026).** The full-quarantine audit ([SHIPPED entry](SHIPPED.md#long-tail-audit--four-scraper-promotions-dresher-the-lab-gray-area-mills-littlefield-july-2026)) promoted the four easy wins and left: **Santa Cruz region decision** gating Kuumbwa Jazz Center (clean Tribe REST, 77 upcoming — the biggest unclaimed prize), Indexical, and Santa Cruz Civic (also 403-blocked); **effortful-but-real calendars** at Make-Out Room (HTTP-only RSS, lineups in post bodies), Meyhouse Jazz (Wix SSR, Peninsula jazz room), ATA (days-ahead posting horizon), The Monkey House (flyer images), Peacock Lounge and Shapeshifters Cinema (hand-edited HTML, low volume); **Arc Gallery** (trivial public-GCal ICS but music is a subset of gallery programming — needs a filter story); and a **human check on Temescal Arts Center** (Yelp says closed June 2026; BI still lists shows). **Unblock conditions:** per-venue — the region decision for the Santa Cruz trio, explicit effort appetite for the yellows, a filter call for Arc.
...
- **Poor House Bistro scraper** (San Jose blues). Music schedule published only as a monthly JPEG; the site's EventPrime calendar AJAX returns zero events; no .ics/JSON/JSON-LD. **Unblock condition:** venue publishes a structured calendar, or OCR/headless appetite. (Café Pink House and Art Boutiki: confirmed CLOSED July 2026 — struck from candidate lists, do not re-sweep.)
