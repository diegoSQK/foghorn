"""Stale-row reaping (``ingest_scraped_shows(prune=True, source_scraper="a_scraper")``).
...
def test_retitled_show_replaces_rather_than_duplicates(
    ...
    ingest_scraped_shows(
        ...
        prune=True, source_scraper="a_scraper",
    )
...
def test_empty_run_never_prunes(conn: sqlite3.Connection, venue: Venue) -> None:
    ...
    ingest_scraped_shows(
        conn,
        venue,
        [_scraped("Real Show", datetime(2026, 9, 1, 20, 0))],
        prune=True,
        source_scraper="a_scraper",
    )
    result = ingest_scraped_shows(conn, venue, [], prune=True, source_scraper="a_scraper")
    assert result.reaped == 0
...
def test_run_with_errors_never_prunes(
    ...
    ingest_scraped_shows(
        conn,
        venue,
        [_scraped("Keeper", datetime(2026, 9, 2, 20, 0))],
        prune=True,
        source_scraper="a_scraper",
    )
    ...
    result = ingest_scraped_shows(conn, bad_venue, [broken], prune=True, source_scraper="a_scraper")
