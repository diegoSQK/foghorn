...
Source: the venue's WordPress site (thechapelsf.com) runs the same
**SeeTickets** white-label plugin as Rickshaw Stop, server-rendering the full
event list into ``/calendar/`` as structured "list event" cards — title, date,
doors/show times, headliners vs supporting talent (already split!), price
range, genre, and the per-event ``wl.seetickets.us`` ticket page. No JS
execution is needed for page one; the remaining pages (this calendar runs
seven) load through the plugin's documented ``admin-ajax.php`` action
(``get_seetickets_events``) that returns the same card HTML, using a nonce
embedded in the calendar page. The card markup matches Rickshaw Stop's flavor
of the plugin (``p.title`` / ``p.date`` / "Supporting Talent: …").

Card dates carry no year ("Tue Jul 7"); the year is inferred forward from
``today`` (the list only shows upcoming events, so a month/day earlier than
today means next year). The calendar covers the whole Chapel complex — cards
may sit "at Curio" (the adjoining bar) or "at The Chapel Outdoor Stage"; those
are kept when they're music (e.g. DJ day parties). Non-shows are dropped: the
World Cup screenings are tagged "Sports General", and "Private Event" hold
cards are matched by name (their genre, "Other Content", is also used on real
music shows, so it can't be the filter).

Runnable standalone: ``python -m foghorn.scrapers.the_chapel`` prints the
...
AJAX_URL = "https://thechapelsf.com/wp-admin/admin-ajax.php"
USER_AGENT = "foghorn-scraper/0.1 (contact via diegoSQK/foghorn issues)"
SCRAPE_WINDOW_DAYS = 90
REQUEST_TIMEOUT = 30.0

# var seetickets_ajax_obj = {"ajax_url":"...","nonce":"628c00b85b"};
...
# "Tue Jul 7" — the weekday is ignored, month is a three-letter abbreviation.
...
# The Chapel's supporting-talent line is "Supporting Talent: …"; strip the
# label either way the plugin renders it ("with …" on some sibling sites).
...
# World Cup screenings ("Sports General").
...
def _parse_date(text: str, today: dt.date) -> dt.date | None:
    """Resolve a year-less "Tue Jul 7" to a concrete date at or after ``today``
