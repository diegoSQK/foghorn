"""Mystic Theatre scraper.

Source: the venue's WordPress site (mystictheatre.com) runs the same
**SeeTickets** white-label plugin as Rickshaw Stop and GAMH, server-rendering
the event list into ``/calendar/`` as structured "list event" cards — title,
date, doors/show times, headliners vs supporting talent (already split!),
price range, ages, genre, and the per-event ``wl.seetickets.us`` ticket page.
No JS execution is needed for page one; the remaining pages load through the
plugin's documented ``admin-ajax.php`` action (``get_seetickets_events``) that
returns the same card HTML, using a nonce embedded in the calendar page.

This site's flavor matches Rickshaw Stop's field classes (``p.title`` /
``p.date`` / ``p.header``, not GAMH's ``p.event-title`` family), but its
supporting-talent line is labeled like GAMH's ("with Jason Beard's Bring Joy")
and sometimes "Featuring: Napoleon Murphy Brock" — so the label-stripping
covers all three spellings. The calendar page also embeds a month-grid widget
(``seetickets-calendar-event-container`` tiles) alongside the list cards; only
the list cards are parsed, so nothing is double-counted.

Card dates carry no year ("Fri Jul 10"); the year is inferred forward from
``today`` (the list only shows upcoming events, so a month/day earlier than
today means next year). Multi-night runs sold as one product render one card
per night with no doors/show time spans (the times live in the title text,
e.g. "9PM Sat / 7:30PM Sun") — those cards are skipped like other time-less
placeholders rather than guessing a start time.

Runnable standalone: ``python -m foghorn.scrapers.mystic_theatre`` prints the
...
VENUE_SLUG = "mystic_theatre"

CALENDAR_URL = "https://mystictheatre.com/calendar/"
AJAX_URL = "https://mystictheatre.com/wp-admin/admin-ajax.php"
USER_AGENT = "foghorn-scraper/0.1 (contact via diegoSQK/foghorn issues)"
SCRAPE_WINDOW_DAYS = 90
REQUEST_TIMEOUT = 30.0

# var seetickets_ajax_obj = {"ajax_url":"...","nonce":"5afcff19d1"};
...
# "Fri Jul 10" — the weekday is ignored, month is a three-letter abbreviation.
...
# Mystic's supporting-talent line reads "with Jason Beard's Bring Joy" or
# "Featuring: Napoleon Murphy Brock"; strip the label however the plugin
# renders it (including Rickshaw's "Supporting Talent: …" spelling).
_SUPPORT_PREFIX_RE = re.compile(
    r"^\s*(?:supporting\s+talent\s*:?\s*|with\s+|featuring\s*:?\s*)", re.IGNORECASE
)
# Genres the venue tags on clearly-non-music programming (Mystic books
# stand-up regularly). Ambiguous tags are kept per house policy.
...
def _parse_date(text: str, today: dt.date) -> dt.date | None:
    """Resolve a year-less "Fri Jul 10" to a concrete date at or after ``today``
...
def parse_html(
    ...
    for card in soup.select("div.seetickets-list-event-container"):
        show_date = _parse_date(_select_text(card, "p.date"), today)
        ...
        title_link = card.select_one("p.title a")
        ...
        if start_time is None:
            continue  # no usable time — placeholder or multi-night pass card
