"""Rickshaw Stop scraper.

Source: the venue's WordPress site runs the **SeeTickets** white-label plugin,
which server-renders the full event list into ``/calendar/`` as structured
"list event" cards — title, date, doors/show times, headliners vs supporting
talent (already split!), price range, ages, genre, and the per-event
``wl.seetickets.us`` ticket page. No JS execution is needed for page one; the
remaining pages load through a documented ``admin-ajax.php`` action
(``get_seetickets_events``) that returns the same card HTML, using a nonce
embedded in the calendar page. There is no ``.ics`` feed and the events are
not WordPress posts, so this HTML-plus-AJAX route is the cleanest available.

Card dates carry no year ("Thu Jul 2"); the year is inferred forward from
``today`` (the list only shows upcoming events, so a month/day earlier than
today means next year).

Runnable standalone: ``python -m foghorn.scrapers.rickshaw_stop`` prints the
...
VENUE_SLUG = "rickshaw_stop"

CALENDAR_URL = "https://rickshawstop.com/calendar/"
AJAX_URL = "https://rickshawstop.com/wp-admin/admin-ajax.php"
USER_AGENT = "foghorn-scraper/0.1 (contact via diegoSQK/foghorn issues)"
SCRAPE_WINDOW_DAYS = 90
REQUEST_TIMEOUT = 30.0

# var seetickets_ajax_obj = {"ajax_url":"...","nonce":"20c8a2f544"};
...
# "Thu Jul 2" — the weekday is ignored, month is a three-letter abbreviation.
...
# ("Other Content") are kept per house policy.
...
def _parse_date(text: str, today: dt.date) -> dt.date | None:
    """Resolve a year-less "Thu Jul 2" to a concrete date at or after ``today``
