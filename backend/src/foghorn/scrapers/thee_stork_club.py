"""Thee Stork Club scraper.

Source: the venue's WordPress site (theestorkclub.com) runs the same
**SeeTickets** white-label plugin as Rickshaw Stop — and the *same flavor* of
it: cards are classed ``p.title`` / ``p.date`` / ``p.headliners``, and the
supporting-talent line reads "Supporting Talent: …". The full event list is
server-rendered into ``/calendar/`` as structured "list event" cards — title,
date, doors/show times, headliners vs supporting talent (already split!),
price, ages, genre, and the per-event ``wl.seetickets.us`` ticket page. No JS
execution is needed for page one; the remaining pages load through the
...
Card dates carry no year ("Fri Jul 3"); the year is inferred forward from
``today`` (the list only shows upcoming events, so a month/day earlier than
today means next year). Club quirks: DJ nights and karaoke cards usually leave
``p.headliners`` empty, so the title-fallback path is routine here, and the
venue tags genres like "DJ/Dance" and "Karaoke" — music-adjacent, kept per
house policy (only clearly-non-music genres are dropped). The calendar also
hosts festival-afterparty cards for the sister room Eli's Mile High Club;
cards whose ``p.venue`` line names another venue are dropped (though beware:
some off-site cards only say so in the title, which is left alone — titles
are too free-form to filter safely).

Runnable standalone: ``python -m foghorn.scrapers.thee_stork_club`` prints the
...
VENUE_SLUG = "thee_stork_club"

CALENDAR_URL = "https://theestorkclub.com/calendar/"
AJAX_URL = "https://theestorkclub.com/wp-admin/admin-ajax.php"
USER_AGENT = "foghorn-scraper/0.1 (contact via diegoSQK/foghorn issues)"
SCRAPE_WINDOW_DAYS = 90
REQUEST_TIMEOUT = 30.0

# var seetickets_ajax_obj = {"ajax_url":"...","nonce":"fbeb1c4846"};
# Anchored to the SeeTickets config object so another plugin's nonce can never
# be grabbed by mistake (same defensive anchor as the GAMH scraper).
...
# "Fri Jul 3" — the weekday is ignored, month is a three-letter abbreviation.
...
_SUPPORT_PREFIX_RE = re.compile(r"^\s*supporting\s+talent\s*:?\s*", re.IGNORECASE)
# Genres the venue tags on clearly-non-music programming. Ambiguous tags
# ("Karaoke", "DJ/Dance", "Other Content") are kept per house policy.
...
def _parse_date(text: str, today: dt.date) -> dt.date | None:
    """Resolve a year-less "Fri Jul 3" to a concrete date at or after ``today``
...
def parse_html(
    ...
    for card in soup.select("div.seetickets-list-event-container"):
        ...
        if genre in _NON_MUSIC_GENRES:
            continue
        # Festival afterparties for Eli's Mile High Club appear on this
        # calendar; when the card's venue line names another room, skip it.
        venue_line = _select_text(card, "p.venue").lower()
        if venue_line and "stork" not in venue_line:
            continue
        ...
        # DJ-night/karaoke cards leave p.headliners empty → title fallback.
